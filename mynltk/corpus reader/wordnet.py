    ...
    def _morphy(self, form, pos, check_exceptions=True):
        ...
        if check_exceptions:
            if form in exceptions:
                # Bingham updates:
                #   Updated sequence to return exceptions first.  If no
                #   exceptions are returned by filter_forms, then try the
                #   original form(s).  This should prevent the rare cases
                #   where both an exception and a form are returned by
                #   filter_forms and the exception is not shorter in length
                #   than the original form.
                results = filter_forms(exceptions[form])
                if results:
                    return results
                else:
                    return filter_forms([form])
